...
db.serialize(() => {
    ...
    db.run("CREATE TABLE IF NOT EXISTS printers (inventoryNumber TEXT PRIMARY KEY, model TEXT, year INTEGER)");
    db.run("CREATE TABLE IF NOT EXISTS students (studentID TEXT PRIMARY KEY, firstName TEXT, lastName TEXT)");
...
app.post('/printers', (req: Request, res: Response) => {
  ...
  db.run("INSERT INTO printers (inventoryNumber, model, year) VALUES (?, ?, ?)", [inventoryNumber, model, year], function(err) {
    if (err) {
      return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: err.message });
    }
...
app.post('/students', (req: Request, res: Response) => {
  ...
  db.run("INSERT INTO students (studentID, firstName, lastName) VALUES (?, ?, ?)", [studentID, firstName, lastName], function(err) {
    if (err) {
      return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: err.message });
    }
    return res.status(StatusCodes.CREATED).json({ id: this.lastID });
...
app.post('/reservations', (req: Request, res: Response) => {
    ...
    if (isNaN(Date.parse(fromDateTime))) {
      return res.status(StatusCodes.BAD_REQUEST).json({ error: "Invalid fromDateTime, must be an ISO 8601 string" });
    }
    
    db.run("INSERT INTO reservations (studentID, inventoryNumber, fromDateTime, durationHours) VALUES (?, ?, ?, ?)", 
        ...
        db.run("DELETE FROM reservations;", error => {
            ...
            db.run("DELETE FROM students;", error => {
                ...
                db.run("DELETE FROM printers;", error => {
                    ...
                    res.status(StatusCodes.NO_CONTENT).json({ message: "All data deleted successfully" });
                });
            });
        });
    });
